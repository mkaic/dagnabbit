...
fn descendants_from_children(children: &[Vec<usize>], node: usize) -> Vec<bool> {
    let mut descendants = vec![false; children.len()];
...
fn can_reach_with_scratch(
    ...
    stack.clear();
    stack.extend(children[src].iter().copied());
...
fn sample_valid_parent(eligible: &[usize], forbidden: &[bool], rng: &mut StdRng) -> usize {
    ...
    let valid_count = eligible
        .iter()
        .filter(|candidate| !forbidden[**candidate])
        .count();
...
fn build_edge_split_graph(
    ...
    while split_idx < num_trunk_nodes {
        ...
        for edge_idx in round_edge_indices {
            ...
            let mut forbidden = descendants_from_children(&children, v);
            forbidden[v] = true;
            let extras: Vec<usize> = (0..in_degree.saturating_sub(1))
                .map(|_| sample_valid_parent(&eligible, &forbidden, rng))
                .collect();
...
fn mcmc_rewire(
    ...
    let mut seen = vec![0; graph.children.len()];
    let mut seen_token = 0;
    ...
    for _ in 0..num_steps {
        if edge_count >= 2 && randrange(rng, 2) == 0 {
            ...
            seen_token += 1;
            let reaches_first_parent = can_reach_with_scratch(
                &graph.children,
                second_child,
                first_parent,
                &mut seen,
            ...
            let mut reaches_second_parent = false;
            if !reaches_first_parent {
                seen_token += 1;
                reaches_second_parent = can_reach_with_scratch(
                    &graph.children,
                    first_child,
                    second_parent,
                    &mut seen,
            ...
            if reaches_first_parent || reaches_second_parent {
                graph.children[first_parent].push(first_child);
                graph.children[second_parent].push(second_child);
                continue;
            }
