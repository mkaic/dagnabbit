...
class WeightAverage:
    """Exponential moving average of a model's parameters, sampled from.

    Routinely the difference between "this does not work" and "this works", and
    the most commonly skipped item on any diffusion checklist. The averaged
    weights are what you evaluate and ship; the live weights only ever take
    gradient steps.

    Buffers are copied rather than averaged: they hold fitted statistics and
    integer counters, not things with a meaningful running mean.
    """
    ...
    def update(self, model: nn.Module) -> None:
        for name, parameter in model.named_parameters():
            self.averaged[name].lerp_(parameter.detach(), 1.0 - self.decay)

    @torch.no_grad()
    def copy_into(self, model: nn.Module) -> dict[str, Tensor]:
        """Swap the averaged weights in, returning the live ones to restore."""
        live = {}
        for name, parameter in model.named_parameters():
            live[name] = parameter.detach().clone()
            parameter.copy_(self.averaged[name])
        return live

    @torch.no_grad()
    def restore(self, model: nn.Module, live: dict[str, Tensor]) -> None:
        for name, parameter in model.named_parameters():
            parameter.copy_(live[name])

    def state_dict(self) -> dict[str, Tensor]:
        return dict(self.averaged)

    def load_state_dict(self, state: dict[str, Tensor]) -> None:
        missing = set(self.averaged) - set(state)
        if missing:
            raise ValueError(f"averaged weights missing keys: {sorted(missing)}")
        for name, tensor in state.items():
            if name in self.averaged:
                self.averaged[name].copy_(tensor)
