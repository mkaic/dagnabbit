...
def create_visualization_plots(shape: Tuple[int], packed: np.ndarray, unpacked: np.ndarray):
    """
    ...
    ax_unpacked.set_title(f'Unpacked Array (shape={shape})', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f'/home/mkaic/dagnabbit/dagnabbit/outputs/bitpacking_test_{shape[0]}x{shape[1]}.png', 
                dpi=150, bbox_inches='tight')
    print(f"   Saved visualization to outputs/bitpacking_test_{shape[0]}x{shape[1]}.png")
    plt.close()
...
def test_roundtrip_correctness(shape: Tuple[int]) -> bool:
    ...
    for coord in test_coords:
        ...
        if tuple(reconstructed_coords) == coord:
            ...
            print(f"   ✗ Coord {coord} -> reconstructed as {tuple(reconstructed_coords)}")
            all_passed = False
